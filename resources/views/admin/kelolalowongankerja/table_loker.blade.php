                        ...
                        <tbody>
                            @foreach ($lowongans as $key => $lowongan)
                                <tr>
                                    ...
                                    <td>{{ ucfirst(str_replace('_', ' ', $lowongan->tipe_kerja)) }}</td> 
                                    <td>{{ $lowongan->tanggal_dibuka }}</td>
                                    <td>{{ $lowongan->tanggal_ditutup }}</td>
                                    <td>
                                        <!-- Tombol aksi -->
                                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#editLowonganModal{{ $lowongan->id }}">Edit</button>
                                        <form action="{{ route('lowongans.destroy', $lowongan->id) }}" method="POST"
                                ...
                                <!-- Modal Edit Lowongan -->
                                <div class="modal fade" id="editLowonganModal{{ $lowongan->id }}" tabindex="-1"
                                    aria-labelledby="editLowonganModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <!-- Form edit lowongan -->
                                                <form action="{{ route('lowongans.update', $lowongan->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="mb-3">
                                                        <label for="judulLowongan" class="form-label">Judul
                                                            Pekerjaan</label>
                                                        <input type="text" class="form-control" name="judul"
                                                            value="{{ $lowongan->judul }}">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="perusahaanLowongan"
                                                            class="form-label">Perusahaan</label>
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="tipeKerja" class="form-label">Tipe Kerja</label>
                                                        <select class="form-control" name="tipe_kerja">
                                                            <option value="kontrak"
                                                                {{ $lowongan->tipe_kerja == 'kontrak' ? 'selected' : '' }}>
                                                                Kontrak</option>
                                                            <option value="paruh_waktu"
                                                                {{ $lowongan->tipe_kerja == 'paruh_waktu' ? 'selected' : '' }}>
                                                                Paruh Waktu</option>
                                                            <option value="full_time"
                                                                {{ $lowongan->tipe_kerja == 'full_time' ? 'selected' : '' }}>
                                                                Full Time</option>
                                                            <option value="magang"
                                                                {{ $lowongan->tipe_kerja == 'magang' ? 'selected' : '' }}>
                                                                Magang</option>
                                                        </select>
                                                    </div>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="date" class="form-control" name="tanggal_dibuka"
                                                            value="{{ $lowongan->tanggal_dibuka }}">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="tanggalDitutup" class="form-label">Tanggal
                                                            Ditutup</label>
                                                        <input type="date" class="form-control" name="tanggal_ditutup"
                                                            value="{{ $lowongan->tanggal_ditutup }}">
                                                    </div>
